...
const RideDetail = () => {
  ...
  const handleBook = async () => {
    try {
      ...
      if (!token) {
        toast("Not logged in", { description: "Please login first." });
        return;
      }
      ...
      toast("Error booking ride", { description: err.response?.data?.error || err.message });
    }
  };

  if (loading) return <Skeleton className="w-full" />;
  if (error) return <h3 className="text-xl p-10 text-center h-svh">Error: {error.message || "Ride not found"}</h3>;

  // Safely handle undefined start/end times
    ...
    <main className="pb-12 md:py-14 px-6 2xl:px-20 2xl:container 2xl:mx-auto">
      <div className="flex flex-col gap-8 md:flex-row justify-center w-full">
        ...
        <div className="flex flex-col justify-start items-start gap-2 w-full">
          ...
          <div className="w-full py-3 border-t">
            <p>Duration: {startTime && endTime ? formatDistance(startTime, endTime) : "-"}</p>
          </div>
          <div className="w-full py-3 border-t">
          ...
          <div className="flex flex-col justify-start items-stretch h-full w-full">
            <div className="flex flex-col justify-start items-start flex-shrink-0">
              <div className="flex w-full space-x-4 py-8 border-b">
                ...
                <div className="flex justify-center items-start flex-col space-y-2">
                  ...
                  <div className="flex items-center text-sm gap-1 text-muted-foreground">
                    <Star fill="yellow" size={20} className="text-transparent" /> {data?.creator.stars || 0} ratings
                  </div>
                </div>
              </div>
              <div className="flex justify-center items-start flex-col space-y-4 mt-8">
                ...
                <p className="text-sm text-muted-foreground">{data?.creator.ridesCreated?.length || 0} Rides published</p>
                <p className="text-sm text-muted-foreground">Member since {data?.creator.createdAt?.substring(0, 4) || "-"}</p>
              </div>
              <div className="flex justify-center items-start flex-col space-y-4 mt-8">
